...
LOCALPATH <- '/Users/u0118120/FlySleepLab Dropbox/Joana/sleep_signature/10x/analysis/shiny/bigdata/3/'
root_dir = LOCALPATH


gene <- as.vector(
  unique(
  read.csv(file.path(root_dir, "data", "combined_nobatch.csv"))$gene))
ct <- as.vector(unique(read.csv(file.path(root_dir, "data", "combined_nobatch.csv"))$cluster))
...
server <- function(input, output, session) {
  ...
  clock_long <- read.csv(file.path(root_dir, "data/merged_clock_all.csv"))
  setDT(clock_long)
  ...
  updateSelectizeInput(session, "gene2", choices = gene, server = TRUE)
      ...
      sleep_wake <- read.csv(file.path(root_dir, "data/combined_nobatch.csv"))
      setDT(sleep_wake)
  ...
  updateSelectizeInput(session, "gene3", choices = gene, server = TRUE)
      ...
      sleep_drive_wide <- read.csv(file.path(root_dir, "data/sleep_drive_exp_r2_p_wide.csv"))
