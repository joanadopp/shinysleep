...
LOCALPATH <- '/Users/Joana/Joana/shiny/version1/'
...
gene <- as.vector(
  unique(
  read.csv(file.path(root_dir, "data", "combined_nobatch_pval_anno.csv"))$gene))
ct <- as.vector(unique(read.csv(file.path(root_dir, "data", "combined_nobatch_pval_anno.csv"))$cluster))
...
ui <- navbarPage("Fly Sleep Single Cell", theme = shinytheme("flatly"),
                 ...
                 tabPanel("about",
                          mainPanel(
                            ...
                            tags$h1("version 1 (this one): all genes in all annotated clusters", style = "font-size:18px"),
                            
                            div(tags$a(href="https://joana-dopp.shinyapps.io/Fly_Sleep_Single_Cell/", "version 2"),
                                ": significant genes in all clusters", style = "font-size:18px"),
...
server <- function(input, output, session) {
  ...
  updateSelectizeInput(session, "gene1", choices = gene, server = TRUE)
  
  clock_long <- read.csv(file.path(root_dir, "data/merged_clock.csv"))
  setDT(clock_long)
  ...
  updateSelectizeInput(session, "gene2", choices = gene, server = TRUE)
      
      sleep_wake <- read.csv(file.path(root_dir, "data/combined_nobatch_pval_anno.csv"))
      setDT(sleep_wake)
  ...
  updateSelectizeInput(session, "gene3", choices = gene, server = TRUE)
      
      sleep_drive_wide <- read.csv(file.path(root_dir, "data/sleep_drive_exp_r2_p_anno_wide.csv"))
